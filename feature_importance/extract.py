...
def task_handler(args):
    ...
    with open(filepath, "r") as f:
        try:
            for x in f:
                ...
                sizes.append(1 if int(x[1]) > 0 else -1)
    ...
    with open(dest, "w") as fout:
        for x in features:
            if isinstance(x, str):
                if '\n' in x:
                    fout.write(x)  # Write strings with newlines as is
                else:
                    fout.write(x + " ")  # Write strings with a space
            else:
                try:
                    # Convert numeric types (e.g., np.float64) to plain numbers
                    if isinstance(x, (float, int)):
                        fout.write(f"{x} ")  # Format as plain number
                    else:
                        fout.write(f"{float(x)} ")  # Convert to float if possible
                except ValueError:
                    fout.write(repr(x) + " ")  # Fallback for non-numeric objects
